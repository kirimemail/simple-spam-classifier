...
class Config(object):
    FLASK_DEBUG = bool(int(os.environ.get('FLASK_DEBUG'))) or False
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get('SQLALCHEMY_DATABASE_URI') or 'sqlite:///' + os.path.join(basedir,
                                                                                                       'app.db')
    SQLALCHEMY_RECORD_QUERIES = bool(int(os.environ.get('SQLALCHEMY_RECORD_QUERIES'))) or False
    SQLALCHEMY_POOL_RECYCLE = int(os.environ.get('SQLALCHEMY_POOL_RECYCLE')) or 299
    SQLALCHEMY_POOL_SIZE = int(os.environ.get('SQLALCHEMY_POOL_SIZE')) or 20
    SQLALCHEMY_MAX_OVERFLOW = int(os.environ.get('SQLALCHEMY_MAX_OVERFLOW')) or 10

    LOG_TO_STDOUT = bool(int(os.environ.get('LOG_TO_STDOUT'))) or True
